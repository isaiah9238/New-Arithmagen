...
import type { LabComponentProps, HypotrochoidCoeffs } from './types';
import { Checkbox } from '@/components/ui/checkbox';
import { Separator } from '@/components/ui/separator';

export const HypotrochoidLab: React.FC<LabComponentProps<HypotrochoidCoeffs>> = ({ title, coeffs, setCoeffs, onClose }) => {
    const isHypocycloid = coeffs.r === coeffs.d;
    const isCurtate = coeffs.d < coeffs.r;
    const isProlate = coeffs.d > coeffs.r;

    let description = 'Circle rolling on another circle\'s interior.';
    if (isHypocycloid) description = 'Common Hypocycloid (d = r)';
    if (isCurtate) description = 'Curtate Hypotrochoid (d < r)';
    if (isProlate) description = 'Prolate Hypotrochoid (d > r)';
            ...
            <CardContent className="space-y-4 pt-2">
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <Label className="text-sm">Fixed Radius (R)</Label>
                    ...
                    <Slider value={[coeffs.R]} onValueChange={(v) => setCoeffs(prev => ({ ...prev, R: v[0] }))} min={1} max={15} step={0.5} />
                </div>
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <Label className="text-sm">Rolling Radius (r)</Label>
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        ...
                        <Input type="number" className="h-8 w-20" value={coeffs.rotations} onChange={(e) => setCoeffs(prev => ({ ...prev, rotations: parseFloat(e.target.value) || 0 }))} step={1} />
                    </div>
                    <Slider value={[coeffs.rotations]} onValueChange={(v) => setCoeffs(prev => ({ ...prev, rotations: v[0] }))} min={1} max={20} step={1} />
                </div>
            </CardContent>
        </Card>
    );
