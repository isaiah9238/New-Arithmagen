...
import type { LabComponentProps, EpitrochoidCoeffs } from './types';
import { Checkbox } from '@/components/ui/checkbox';
import { Separator } from '@/components/ui/separator';

export const EpitrochoidLab: React.FC<LabComponentProps<EpitrochoidCoeffs>> = ({ title, coeffs, setCoeffs, onClose }) => {
    const isEpicycloid = coeffs.r === coeffs.d;
    const isCurtate = coeffs.d < coeffs.r;
    const isProlate = coeffs.d > coeffs.r;

    let description = 'Circle rolling on another circle\'s exterior.';
    if (isEpicycloid) description = 'Common Epicycloid (d = r)';
    if (isCurtate) description = 'Curtate Epitrochoid (d < r)';
    if (isProlate) description = 'Prolate Epitrochoid (d > r)';
        ...
        <Card className="relative">
            ...
            <CardContent className="space-y-4 pt-2">
                ...
                <div className="space-y-2">
                    ...
                    <Slider value={[coeffs.R]} onValueChange={(v) => setCoeffs(prev => ({ ...prev, R: v[0] }))} min={1} max={10} step={0.5} />
                </div>
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
